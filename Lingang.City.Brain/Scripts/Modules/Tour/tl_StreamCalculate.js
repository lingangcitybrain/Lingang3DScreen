define(["config", "common", "util"], function (con, com, util) {
    return {
        ...
        //人流预测数值
        peopleCount: [248, 1026, 1232, 2116, 4507],
        //人流预测当前数组值
        k:0,
        //加载人流预测信息
        ListData: new util.HashMap,//缓存
        loadStream:function()
        {
        ...
        //加载动线
        loadStreamLine:function(flag)
        {
            if (flag == 1) {
                map.loadArea("renliudongxian");
            } else {
                map.unloadArea("renliudongxian");
            }       
        },
        //加载中间列
        loadCenter: function () {
        ...
        loadPoi:function()
        {
            ...
            for (var i = 0; i < this.POIData.length; i++) {
                ...
                if (row.stationtype == "4") { iconSize = Q3D.vector2(140, 202); }
        ...
        alternatePlay:function()
        {
            ...
            // 15分, 30分,1小时，6小时 ，明日
            if (this.k >= this.peopleCount.length){this.k = 0;}

            var count = this.peopleCount[this.k]
            var lastcount = 0;
            if (this.k != 0) { lastcount = this.peopleCount[this.k - 1] }
        ...
        wlrlycChartFun: function (wlrlycK){
           ...
           // var yAxisText = ['', 1523, 4512, 7869, 30151, 31267, ''];
            ...
            yAxisText = []
            yAxisText.push('')
            for (var i = 0; i < this.peopleCount.length; i++) {
                yAxisText.push(this.peopleCount[i])
            }
            yAxisText.push('')
                ...
                seriesDataNormalOption: function(value){
                    return {
                        ...
                        itemStyle:{
                            ...
                            shadowOffsetX: 0,
                            shadowOffsetY: 0,
                            color: { type: 'radial', x: .5, y: .5, x2: 0, y2: 0,
                                colorStops: [{
                                    offset: .2, color: '#c275fe' 
                                }, {
                                    offset: 1, color: '#4d0784' 
                                }],
                                global: false 
                            }
                        },
                    ...
                    return {
                        ...
                        itemStyle:{
                            ...
                            color: { type: 'radial', x: .5, y: .5, x2: 0, y2: 0,
                                colorStops: [{
                                    // offset: .2, color: '#c275fe' 
                                    offset: .2, color: wlrlycWarnLevelColor 
                                }, {
                                    offset: 1, color: '#4d0784' 
        ...
        warmPoiJump:function(totals)
        {
            ...
            */
            ...
            if (totals>28000){ icon="Texture/Common/alarm_yellow.png" }
            if (totals>33000){ icon="Texture/Common/alarm_orang.png" }
            if (totals>45000){ icon="Texture/Common/alarm_red.png"}
            ...
            if (totals > 28000) {
                ...
                if (node) {
                    ...
                    if(poinode){map.setBatchPOIJump(jumppoilist, 100);}
                }
            }else{
        ...
        loadPoiNew: function (timetype)
        {
            ...
            for (var i = 0; i < poidata.length; i++) {
               ...
               // 
                ...
                var poi = { POIName: poiName, Position: position, Text: num, Icon: icon, IconSize: iconSize, FontColor: "#efcfff", FontSize: 14 };
                var node = map.getSceneNode(areaName + "/" + poiName);
                if (node) {
                    node.setVisible(1);
                    ...
                    if (childnode) { childnode.setVisible(1); } else {
                        //创建子POI显示
        ...
        lineControl:function(timetype)
        {
            ...
            //15分钟
            if (timetype == 1){
                if (layer1) { layer1.setVisible(1) }
            }

            //30分钟
            if (timetype == 2) {
                if (layer2) { layer2.setVisible(1) }
            }

            //60分钟
            if (timetype == 3) {
                if (layer3) { layer3.setVisible(1) }
            }
        ...
        lineClose:function()
        {
            ...
            var layer2 = mapObj._map3d.getWorldManager().getLayer("30miao")
            var layer3 = mapObj._map3d.getWorldManager().getLayer("60miao")
            if (layer1) { layer1.setVisible(0) }
            if (layer2) { layer2.setVisible(0) }
            if (layer3) { layer3.setVisible(0) }
        },
        //数值区间取值
        ...
        clearPoiNew:function()
        {
            if (this.ListData.size() > 0) {
                for (i = 0; i < this.ListData.size() ; i++) {
                    ...
                    if (node) { map.destroySceneNode(con.AreaName, nodeName); }

                    var childnodeName = "PoiStreamStationName" + key;
                    var childnode = map.getSceneNode(con.AreaName, childnodeName);
                    if (childnode) { map.destroySceneNode(con.AreaName, childnodeName); }
                }
            }
        },
        //hide
        hidePoiNew: function () {
            if (this.ListData.size() > 0) {
                for (i = 0; i < this.ListData.size() ; i++) {
                    ...
                    var childnodeName = "PoiStreamStationName" + key;
                    var childnode = map.getSceneNode(con.AreaName, childnodeName);
                    if (childnode) { childnode.setVisible(0); }
                }
            }
        },
        //获取POI图标
